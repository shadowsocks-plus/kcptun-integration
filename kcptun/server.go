...
*/
	...
	pass := pbkdf2.Key([]byte("SSplus_Key_NOTsecret"), []byte(SALT), 4096, 32, sha1.New)

	block,_ := kcp.NewAESBlockCrypt(pass)
	lis, err := kcp.ListenWithOptions(listenAddr, block, /*c.Int("datashard")*/10, /*c.Int("parityshard")*/3)
	if err != nil {
		log.Fatal(err)
	}
	log.Println("kcptun server listening on",listenAddr)
...
*/
	for {
		if conn, err := lis.Accept(); err == nil {
			...
			conn.SetMtu(/*c.Int("mtu")*/1350)
			conn.SetWindowSize(/*c.Int("sndwnd")*/1024, /*c.Int("rcvwnd")*/1024)
			conn.SetACKNoDelay(/*c.Bool("acknodelay")*/false)
			conn.SetDSCP(/*c.Int("dscp")*/0)

//			if c.Bool("nocomp") {
//				go handleMux(conn, c.String("target"))
//			} else {

			go handleMux(newCompStream(conn), targetAddr)
